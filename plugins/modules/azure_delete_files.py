...
def delete_files(self, _share, _source_path, _source_file, ):
    found_files = []
    ...
    try:
        ...
        if status:
            ...
            if len(share_exist) != 1:
                status = False
                msg_ret = f"Invalid File Share name: <{_share}>. File does not exist in Account Storage <{self.account_name}>"
