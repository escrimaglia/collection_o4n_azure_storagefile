    ...
    try:
        ...
        if status:
            ...
            if len(found_files) > 1:
                ...
                status=True
                msg_ret={
                    "msg": f"File <{found_files[0]}> downloaded to Directory </{_local_path}> from share <{_share}>. File pattern <{_files}>"}
            else:
                status=False
                msg_ret={
                    "msg": f"File <{found_files}> not downloaded to Directory </{_local_path}> from share <{_share}>. No file to download, File pattern <{_files}>"}
        else:
            msg_ret=f"Invalid Directory: </{_source_path}> in File Share <{_share}>"
            status=False
    except Exception as error:
        msg_ret={"msg": f"File <{found_files}> not downloaded to Directory </{_local_path}>. File pattern <{_files}>", "error": f"<{error}>"}
        status=False
